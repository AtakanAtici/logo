...
function repeatForm() {
    ...
    $('.quantity').on('input', function (e) {
        let per_price = form_clone.getElementsByClassName('per_price')[0].value;
        let without_tax = e.target.value * per_price.replace('₺', '').replace(".", "").replace(',', '.');
    ...
    $('.per_price').on('change', function (e) {
        let without_tax = e.target.value.replace('₺', '').replace(".", "").replace(',', '.') * form_clone.getElementsByClassName('quantity')[0].value;
        let tax = form_clone.getElementsByClassName('tax_percent')[0].value;
        let total = (without_tax * tax / 100) + without_tax;
        form_clone.getElementsByClassName('total_price')[0].value = total;
    });
    $('.tax_percent').on('change', function (e) {
        let without_tax = form_clone.getElementsByClassName('per_price')[0].value.replace('₺', '').replace(".", "").replace(',', '.') * form_clone.getElementsByClassName('quantity')[0].value;
        let tax = form_clone.getElementsByClassName('tax_percent')[0].value;
        let total = (without_tax * tax / 100) + without_tax;
        form_clone.getElementsByClassName('total_price')[0].value = total;
    });
    ...
    $('.select2').select2();
    $('.per_price').mask('000.000.000.000.000,00₺', { reverse: true });
